...
BUILD_FLAGS = -ldflags "-X github.com/renlulu/tendermint/version.GitCommit=`git rev-parse --short=8 HEAD`"
...
draw_deps:
	...
	@goviz -i github.com/renlulu/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
...
rpc-docs:
	...
	godoc2md -template rpc/core/doc_template.txt github.com/renlulu/tendermint/rpc/core | grep -v -e "pipe.go" -e "routes.go" -e "dev.go" | sed 's,/src/target,https://github.com/renlulu/tendermint/tree/master/rpc/core,' >> $(DESTINATION)
